<!-- BEGIN: MAIN -->
			<div class="row-fluid">
				<div class="block">
					<h5><i class="icon-group"></i> {PHP.L.usermanager_edit_title} {USERMANAGER_NICKNAME}</h5>
{FILE "{PHP.cfg.themes_dir}/admin/{PHP.cfg.admintheme}/warnings.tpl"}
					<div class="btn-group">
						<a href="{USERMANAGER_URL_FOR_DELETED}" class="btn btn-primary confirmLink confirm">{PHP.L.Delete}</a>
						<a href="{USERMANAGER_URL_FOR_RESET}" class="btn btn-primary confirm">{PHP.L.usermanager_reset_pass}</a>
<!-- IF {PHP.cot_modules.pm} -->
						<a title="{PHP.L.users_sendpm}" href="{USERMANAGER_ID|cot_url('pm','m=send&to=$this')}" class="btn btn-primary">{PHP.L.users_sendpm}</a>
<!-- ENDIF -->
						<a href="{PHP|cot_url('admin','m=other&p=usermanager')}" class="btn btn-success">Назад к списку</a>
					</div>
