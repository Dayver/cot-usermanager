<!-- BEGIN: MAIN -->
			<div class="row-fluid">
				<div class="block">
					<h5><i class="icon-group"></i> {PHP.L.usermanager_details_title} {USERMANAGER_NICKNAME}</h5>
{FILE "{PHP.cfg.themes_dir}/admin/{PHP.cfg.admintheme}/warnings.tpl"}
					<div class="btn-group">
						<a href="{USERMANAGER_URL_FOR_DELETED}" class="btn btn-primary confirmLink confirm">{PHP.L.Delete}</a>
						<a href="{USERMANAGER_URL_FOR_EDIT}" class="btn btn-primary">{PHP.L.Edit}</a>
						<a href="{USERMANAGER_URL_FOR_RESET}" class="btn btn-primary confirmLink confirm">{PHP.L.usermanager_reset_pass}</a>
<!-- IF {PHP.cot_modules.pm} -->
						<a title="{PHP.L.users_sendpm}" href="{USERMANAGER_ID|cot_url('pm','m=send&to=$this')}" class="btn btn-primary">{PHP.L.users_sendpm}</a>
<!-- ENDIF -->
						<a href="{USERMANAGER_USER_URL}" class="btn btn-success">Назад к списку</a>
					</div>
<!-- BEGIN: USERMANAGER_DETAILS -->
					<table class="table table-bordered table-hover">
						...
						<tr>
							...
							<td>{PHP.L.Maingroup}:{USERMANAGER_GROUPS}</td>
						</tr>
						<tr>
							<td>{PHP.L.Country}:</td>
